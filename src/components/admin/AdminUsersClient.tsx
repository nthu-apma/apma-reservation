                ...
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge className={`text-xs ${ROLE_COLORS[user.role]}`}>
                      {(t.admin.role as Record<string, string>)[user.role]}
                    </Badge>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {lang === 'zh' ? `${user._count.reservations} 次預約` : `${user._count.reservations} reservations`}
      ...
      <Dialog open={!!confirmUser} onOpenChange={(open) => { if (!open) setConfirmUser(null) }}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {lang === 'zh'
                ? `確定要刪除「${confirmUser?.name}」（${confirmUser?.email}）嗎？此操作無法復原，該用戶的所有預約紀錄也會一併刪除。`
                : `Are you sure you want to delete "${confirmUser?.name}" (${confirmUser?.email})? This cannot be undone and all their reservation records will also be deleted.`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
